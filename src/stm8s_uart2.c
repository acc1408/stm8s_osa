...
inputchar_t UART2_RxFunc=0; // ”казатель на функцию дл€ приема данных
...
void UART2_Init(uint32_t BaudRate, UART2_WordLength_TypeDef WordLength, UART2_StopBits_TypeDef StopBits, UART2_Parity_TypeDef Parity, UART2_SyncMode_TypeDef SyncMode, UART2_Mode_TypeDef Mode)
{
  ...
  assert_param(IS_UART2_SYNCMODE_OK((uint8_t)SyncMode));
	...
	// инициализаци€ буфера дл€ передачи данных
	bufferInit(&buf, // переменна€ буфер дл€ передачи
						mas, // буфер
...
/**
  * @brief  Configures the UART2Тs IrDA interface.
...
/**
  * @brief  Enables or disables the UART2Тs IrDA interface.
...
// ‘ункци€ подставл€ет в обработчик прерываний
// функцию дл€ захвата вход€щего символа
...
// функци€ дл€ передачи символа в буфера обмена
...
// функци€ дл€ получени€ символа из буфера обмена 
// вызываетс€ в прервании
